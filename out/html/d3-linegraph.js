...
d3.linegraph = function(noTicks, noDots, parties, partyColors, partyNames, dataMax, dataMin, additionalMonths) {
    /* params */
    if (!parties) {
        parties = ['spd', 'cdu', 'csu', 'fdp', 'greens', 'other'];
    }
    if (!partyColors) {
        partyColors = {'spd': '#E3000F', 'cdu': '#000', 'csu': '#008AC5', 'fdp': '#FFED00', 'greens': '#409A3C', 'other': '#a0a0a0'};
    }
    if (!partyNames) {
        partyNames = {'spd': 'SPD', 'cdu': 'CDU', 'csu': 'CSU', 'fdp': 'FDP', 'greens': 'Die Grünen', 'other': 'Others'};
    }
    if (!additionalMonths) {
        additionalMonths = 10;
    }
